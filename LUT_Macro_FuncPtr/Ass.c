#include "Ass.h"
#include <stdio.h>

#define SWAP_MACRO(a, b) do { \
    int temp = a; \
    a = b; \
    b = temp; \
} while (0)

void swap_func(int *a, int *b) {
    int temp = *a;
    *a = *b;
    *b = temp;
} 

unsigned int reverseBitLUT(unsigned int n){
    static const unsigned char bit_reverse_lut[256] = {
    
    };

    unsigned int reversed = 0;
    reversed |= bit_reverse_lut[n & 0xFF] << 24;
    reversed |= bit_reverse_lut[(n >> 8) & 0xFF] << 16;
    reversed |= bit_reverse_lut[(n >> 16) & 0xFF] << 8;
    reversed |= bit_reverse_lut[(n >> 24) & 0xFF];
    return reversed;
}

int swap_with_func(int arr[], int n) {
    int i, j;
    for (i = 0; i < n - 1; i++) {
        for (j = 0; j < n - i - 1; j++) {
            if (arr[j] > arr[j + 1]) {
                swap_func(&arr[j], &arr[j + 1]);
            }
        }
    }
    return 0;
}

void sort_with_macro(int arr[], int n){
    int i, j;
    for (i = 0; i < n - 1; i++) {
        for (j = 0; j < n - i - 1; j++) {
            if (arr[j] > arr[j + 1]) {
                SWAP_MACRO(arr[j], arr[j + 1]);
            }
        }
    }
}

static unsigned char char_to_val(char c) {
    if (c == '\0') return 0;
    return c - 'a' + 1;
}

void compress_bitwise(const char *str, unsigned char *compressed) {
    int i = 0, j = 0;
    unsigned char left, right;
    
    while (str[i] != '\0') {
        left = char_to_val(str[i]);
        right = 0;
        
        if (str[i+1] != '\0') {
            right = char_to_val(str[i+1]);
            i += 2;
        } else {
            i += 1;
        }
        
        compressed[j] = (unsigned char)((left << 4) | right);
        j++;
    }
}

void compress_bitfield(const char *str, CharPair *compressed) {
    int i = 0, j = 0;
    unsigned char left, right;
    
    while (str[i] != '\0') {
        left = char_to_val(str[i]);
        right = 0;
        
        if (str[i+1] != '\0') {
            right = char_to_val(str[i+1]);
            i += 2;
        } else {
            i += 1;
        }
        
        compressed[j].left = left;
        compressed[j].right = right;
        j++;
    }
}

void generic_bubble_sort(int *arr, int size, int (*CR)(int, int)) {
    int i, j;
    for (i = 0; i < size - 1; i++) {
        for (j = 0; j < size - i - 1; j++) {
            if (CR(arr[j], arr[j + 1])) {
                swap_func(&arr[j], &arr[j + 1]);
            }
        }
    }
}